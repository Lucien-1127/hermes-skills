---
name: zhiyan-legal
description: 智研AI法律工作站 v3.07。七層架構（SRP→L0→L0.7白話RAG→L0.8案例驗證→MODE_ROUTER→功能模組→Citation v2.1）。TYPE-S 強制審查。子代理並行 + 法院合規書狀產生。
user-invocable: true
---
...
> 相關技能：`legal-citation-qa`（法規引用正確性六層防禦）
...
3. **確認格式**— 使用 `doc_generator.py` 產生書狀，格式依民事訴訟書狀規則 §3：A4 / 2.5cm 邊界 / 標楷體 14pt / 固定行高 28pt / 頁碼置中底部。完整規範見 `docs/60_概念詞條/書狀格式規範.md`
4. **草擬**— 三段結構：事實摘要 → 法律依據 → 具體請求
5. **TYPE-S 自我校驗**— 檢查法條正確性、事實具體程度、期間合理性、附件完整性。若涉及跨法規引用，先載入 companion skill `legal-citation-qa` 做 pcode 驗證與語境指紋比對
6. **送件前檢查清單**— 逐項確認空白欄位、簽章、附件
7. **交付或寄送**
...
### 子代理並行策略（LEGAL_WRITER + COURTROOM 模式適用）

耗時環節可透過 Hermes `delegate_task` 平行化，實作見 `src/zhiyan_legal/sub_agent.py`：

| 模式 | 說明 | 加速比 |
|------|------|:------:|
| `parallel_citation_verify()` | 條文 + 判決 + 實務三路並行查詢 | 3× |
| `courtroom_parallel()` | 法官/檢察官/辯護人三方獨立準備 | 3× |
| `type_s_review()` | 獨立 QA 子代理審查草稿 | 品質↑ |
| `parallel_legal_research()` | 按法域拆給專門子代理 | N× |
| `parallel_rag_online()` | 本地 RAG + 聯網平行查詢 | 2× |

用法（從 Hermes agent runtime 呼叫）：
```python
from src.zhiyan_legal.sub_agent import parallel_citation_verify
results = parallel_citation_verify("民法", 184)
```

注意：`sub_agent.py` 依賴 `hermes_tools.delegate_task`，只能在 Hermes agent session 內部執行，不能用獨立 Python 執行。設計文件見 `docs/10_核心控制層/17_子代理並行策略.md`。
...
| 類別 | 路徑 |
|------|------|
| 子代理策略設計 | `docs/10_核心控制層/17_子代理並行策略.md` |
| 書狀格式規範 | `docs/60_概念詞條/書狀格式規範.md` |
| 子代理實作 | `src/zhiyan_legal/sub_agent.py` |
| 書狀產生器 | `src/zhiyan_legal/doc_generator.py` |
| 司法院官方範本 | `templates/民事書狀範本.docx` |
