---
name: zhiyan-legal
description: 智研AI法律工作站 v3.05 — 法律分析、查核與研究系統。六層架構：SRP→L0→L0.7 白話RAG（每日3AM自動sync）→MODE_ROUTER→功能模組→Citation v2.1 RAG整合版。
user-invocable: true
---
...
L0 四法融合 → 提取檢索關鍵詞
    ↓
L0.7 查詢本地白話 RAG（python3 rag.py "關鍵詞" --top-k 5 --json）
...
python3 ~/.hermes/rag/legal_translation/rag.py "查詢詞" --json  # 程式解析用
```
...
| 前綴 | 來源 | 範例 |
|:----:|------|:----:|
| [N] | 聯網查證（官方資料庫、判決書） | [1] 全國法規資料庫 |
| [T] | 本地白話翻譯 RAG | [T1] [T2] |

### 行文規則

- **Inline 標記**：首次出現標 [N] 或 [T1]，同段重複不再標
- **段落末尾**：必須列出【本段資料來源】表格，含前綴說明
